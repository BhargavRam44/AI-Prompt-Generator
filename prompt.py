...
class OllamaPromptGenerator:
    ...
    def is_ollama_ready(self):
        try:
            response = requests.get("http://localhost:11434/api/tags", timeout=2)  # Reduced timeout
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not any(model.get("name") == model_name for model in models):
                    print(f"Model '{model_name}' is not detected, but skipping download as per user request.")
    ...
    def generate_prompt(self, description):
        ...
        payload = {
            ...
            "stream": False,
            "options": {
                "num_predict": 75,  # Reduced from 100 for faster response
                "temperature": 0.7,  # Increased for more creative responses
                "top_k": 40,        # Increased for better variety
                "top_p": 0.9,       # Increased for better quality
                "repeat_penalty": 1.1,  # Add repeat penalty for better coherence
                "stop": ["###"]     # Add stop token for more concise responses
            }
        }

        try:
            response = requests.post(self.model_url, json=payload, timeout=15)  # Reduced timeout
            response.raise_for_status()
            result = response.json().get("response", "No response generated.")
            self.cache[cache_key] = result  # Cache the response
            return result
        except requests.RequestException as e:
            return f"Error: {e}"
